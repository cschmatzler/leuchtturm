# @chevrotain/oxlint-plugins - Custom Oxlint Plugins
...
| --------------------- | --------------------- | -------------------------------------------------------------------------- |
| `no-relative-imports` | `no-relative-imports` | Disallows `./` and `../` imports, requires `@chevrotain/*` package imports |
...
```bash
pnpm --filter @chevrotain/oxlint-plugins build
```
