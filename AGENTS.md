...
# Database (from root)
pnpm --filter @roasted/core exec drizzle-kit generate --name migration_name
pnpm --filter @roasted/core exec drizzle-kit push      # Apply to dev DB
...
- **Custom rule**: `no-relative-imports/no-relative-imports` - must use `@roasted/*` imports
...
2. Internal packages (`"@roasted/core"`, `"@roasted/zero"`)
...
| Relative imports across packages           | Use `@roasted/*` package imports         |
...
| `DATABASE_URL`       | `postgres://postgres:postgres@localhost:34601/roasted` | PostgreSQL              |
| `CLICKHOUSE_URL`     | `http://localhost:34602`                               | Analytics DB            |
| `ZERO_APP_ID`        | `roasted`                                              | Zero app identifier     |
