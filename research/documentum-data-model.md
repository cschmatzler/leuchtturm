...
dm_sysobject subtypes include additional system and application types such as:
...
Repeating attributes such as `r_version_label` and `r_folder_id` are exposed through the `_r` table, not the `_s` table.
...
- **Symbolic labels**: Labels such as "CURRENT" are repeating labels applied to the latest/current object, but position/order should not be treated as a stable schema guarantee.
...
ACLs are assigned to objects through object-level ACL attributes such as `acl_name` and `acl_domain`; retention/inheritance behavior is controlled by additional internal attributes and repository policy.
