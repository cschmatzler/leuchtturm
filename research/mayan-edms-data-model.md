...
| ------------------- | ------------------------------------ | ---------------------------------- |
| `id`                | PK (auto)                            |                                    |
| `uuid`              | UUIDField(default=uuid.uuid4)        | Generated document UUID            |
...
`TrashedDocument` is a Django proxy model over `Document`, not a separate database table. It uses `TrashCanManager` to expose documents where `Document.in_trash=True`; trash state is stored directly on `documents_document` through `in_trash` and `trashed_date_time`.

### RecentlyAccessedDocument / RecentlyCreatedDocument

`RecentlyAccessedDocument` is a real model with `user`, `document`, and `datetime_accessed` fields. `RecentlyCreatedDocument` is a proxy over `Document` with a manager for quick filtering.
