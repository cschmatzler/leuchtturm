...
const memoryValidationTools = new Set(["cog_mem_reinforce", "cog_mem_flush", "cog_mem_verify"]);
...
function rememberDebugSummary(state, args) {
	const parts = [
		...
		firstString(args.expression),
		firstString(args.program),
	].filter(Boolean);
...
export default async () => ({
	...
	"tool.execute.before": async (input, output) => {
		...
		if (
			(state.pendingLearning || state.pendingConsolidation) &&
			!memoryWriteTools.has(input.tool) &&
			!memoryConsolidationTools.has(input.tool) &&
		...
		if (memoryWriteTools.has(input.tool)) {
			...
			if (
				looksGenericDefinition(args) ||
				weakRelation(args) ||
				(state.pendingRationale &&
					!/because|why|constraint|invariant|reason|workflow|architecture/i.test(
						String(args.definition || ""),
					))
			) {
				output.system = output.system || [];
				output.system.push(
